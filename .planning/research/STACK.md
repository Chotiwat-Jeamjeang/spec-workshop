...
**Domain:** Lightweight Node.js/Express web app — no-login QR-based waste reporting with AI image classification and LINE notifications (university campus, single JSON-file datastore)
**Researched:** 2026-08-18
**Confidence:** MEDIUM-HIGH (core packages verified live against the npm registry today; qualitative "why"/comparison claims are WebSearch-sourced and cross-checked across 2+ independent results per topic — no Context7/MCP provider was available in this session, so nothing here reaches the HIGH/"official docs fetched via MCP" tier)
...
npm install file-type sharp write-file-atomic proper-lockfile qrcode @line/bot-sdk express-rate-limit sanitize-html helmet
...
# Dev dependencies
npm install -D nodemon eslint prettier supertest
```
...
**If the app will only ever run as a single Node process (typical for a campus-scale intranet tool on one server):**
...
**If report photo volume grows and disk/JSON-file size becomes a concern:**
...
**If the university later wants staff to get a richer LINE message (photo thumbnail, "mark done" quick-reply button):**
...
---
*Stack research for: Lightweight Node.js/Express campus waste-reporting app (QR + AI vision + LINE notifications, JSON file storage)*
*Researched: 2026-08-18*
